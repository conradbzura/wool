...
class TestContext:
    ...
    def test_from_protobuf_with_corrupt_value(self):
        ...
        # Assert
        assert var not in ctx._data, (
            "Corrupt entry should be skipped, not partially decoded"
        )
    ...
    def test_from_protobuf_with_malformed_id_emits_warning(self):
        ...
        # Assert
        assert isinstance(ctx._id, uuid.UUID), (
            "Fallback chain id should be a freshly-minted UUID"
        )
        assert ctx._id.hex != "not-a-uuid", "Malformed id should not be propagated"
    ...
    async def test_from_protobuf_in_caller_task(self):
        ...
        Then:
            ``context_registry.get()`` from inside the task should still
            return None after the call — decoding a wire frame must not
