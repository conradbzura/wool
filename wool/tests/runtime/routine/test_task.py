...
def test_passthrough_key_eq_with_foreign_object():
    """Test :func:`operator.eq` over a passthrough store key against a
    foreign object.

    Given:
        A passthrough store key and a non-key object (an int and a
        string).
    When:
        The ``==`` operator is evaluated on the key against the
        foreign objects.
    Then:
        It should return ``False`` for both — the key never equates
        to a foreign type even when the operator falls back to the
...
class TestPassthroughSerializer:
    ...
    def test_loads_serializer_storage_pruning(self):
    ...
    def test_streaming_dump_load_cycle_key_bounds(self):
    ...
    @pytest.mark.asyncio
    async def test_pool_with_same_task_id(self):
    ...
    @pytest.mark.asyncio
    async def test_pool_when_refcount_drops_to_zero(self):
