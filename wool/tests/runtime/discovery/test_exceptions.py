...
def test___init___should_subclass_wool_error_not_runtime_error(cls):
    """Test each discovery namespace exception joins the Wool umbrella.
    ...
    Then:
        It should be caught by that clause, be the very class re-exported
        from wool, and not subclass RuntimeError — a single except clause
        must match every Wool signal.
    """
    # Arrange
    raised = cls("ns")
