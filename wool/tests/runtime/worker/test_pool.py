...
class TestWorkerPool:
    ...
    async def test___aenter___should_raise_when_discovery_satisfies_neither_protocol(
        ...
        Then:
            ...
            the message the spawning arm emits names only one, so the
            two arms stay distinguishable to a caller.
        """
        # Arrange
    ...
    async def test___aenter___should_accept_a_subscriber_when_no_workers_spawned(
        ...
        """
        # Arrange — an iterable with no publisher half
    ...
    async def test___aenter___should_raise_when_spawning_discovery_lacks_a_publisher(
        self, mocker: MockerFixture, mock_worker_proxy, mock_local_worker, discovery
    ):
        ...
        When:
            The pool is entered
        Then:
            It should raise TypeError naming DiscoveryLike alone — a
            pool that spawns workers announces them, so a subscriber
            without a publisher is not enough.
        """
        # Act & assert
    ...
    async def test___aenter___should_exit_a_rejected_discovery_context(
        self, mocker: MockerFixture, mock_worker_proxy, mock_local_worker, spawn
    ):
        ...
        Then:
            It should have exited the context it entered — the
            rejection happens inside the resolved block, so a factory
            that produced the wrong thing is unwound with the failure.
        """
        # Arrange
        discovery = _NoProtocolDiscovery()
