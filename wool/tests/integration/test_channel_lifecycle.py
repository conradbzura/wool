...
their channels either way, through the stop RPC (see
`wool.LocalWorker.stop`); those tests pin the symptom end to end. The
arrangements that fail without the proxy's channel-pool hold are the
ones over an *externally* started worker the pool never shuts down, and
the one reading a worker's own counters as its cached proxy retires.
"""
...
class TestChannelPoolLifecycle:
    ...
    async def test___aexit___should_close_pooled_channels_when_last_pool_exits(
        ...
        Then:
            It should leave the channel pool caching nothing.
        """
    ...
    async def test_clear_channel_pool_should_warn_when_loop_stops_without_a_proxy(
        ...
        Given:
            A running worker polled for its idle duration by a bare
...
@pytest.mark.integration
class TestWorkerChannelRetirement:
    @pytest.mark.asyncio
        ...
        async def body():
            ...
            async with build_pool_from_scenario(
                ...
                retired = await poll_until(
                    ...
                    lambda counters: counters == (0, 0, 0),
            ...
            # Assert
            assert warm == (1, 0, 1)
            assert retired == (0, 0, 0)

        await retry_grpc_internal(body)


@pytest.mark.integration
class TestLateChannelRelease:
    @pytest.mark.asyncio
