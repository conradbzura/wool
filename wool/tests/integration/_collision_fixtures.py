"""Isolated collision-probe routines for the sibling-collision test.
...
# Regex pattern matching the wool.ContextVarCollision message text
# for the colliding key. The collision message renders the
# (namespace, name) tuple via ``{key!r}`` so the literal substring
# contains regex meta-characters; ``re.escape`` makes it safe to
# pass to ``pytest.raises(..., match=...)``.
COLLIDING_KEY = _re.escape(repr((_COLLISION_KEY_NAMESPACE, _COLLISION_KEY_NAME)))
