...
class TestWoolContextAcrossWorkers:
    ...
    async def test_seeded_context_dispatch_propagates_var_bindings(
        ...
        When:
            The caller invokes the dispatch under the seeded Context
            via ``Context.run_async``
        Then:
        ...
        async def body():
            ...
            async with build_pool_from_scenario(scenario, credentials_map):
                ...
                async def dispatch_under_seed():
                    return await routines.get_tenant_id()

                result = await seed.run_async(dispatch_under_seed)
