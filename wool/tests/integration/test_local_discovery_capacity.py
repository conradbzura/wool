...
class TestLocalDiscoveryCapacity:
    ...
    async def test___aenter___should_raise_when_spawn_exceeds_capacity(self):
        ...
        Then:
            ...
            RuntimeError matching "No available slots", and leave no
            spawned worker process alive.
        """
        # Arrange
        ...
        try:
            ...
            assert any(
                isinstance(leaf, RuntimeError) and "No available slots" in str(leaf)
                for leaf in leaves
            ), f"expected a capacity RuntimeError, got: {leaves!r}"
    ...
    async def test___aenter___should_raise_when_non_owner_exceeds_owner_capacity(self):
        ...
        Then:
            It should abort entry with an ExceptionGroup carrying a
            RuntimeError matching "No available slots" and leave no worker
            process alive — the owner's stamped cap of one governs and the
            non-owner's declared 128 is ignored.
        """
