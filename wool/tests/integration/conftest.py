...
#: The D3 members `_resolve_borrower` can supply a borrowed subscriber
...
def _resolve_borrower(namespace, factory):
    ...
    match factory:
        ...
        case DiscoveryFactory.LOCAL_ASYNC_CM:
            ...
            return _acm()
        case _:
            raise ValueError(f"Unsupported factory for borrower: {factory}")
...
async def _durable_borrowed_pool_context(
    ...
    """Create a DURABLE pool that borrows an externally owned namespace.
    ...
    model.
    """
    namespace = f"borrowed-{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        try:
            ...
            async with publisher:
                await publisher.publish("worker-added", worker.metadata)
                borrower = _resolve_borrower(namespace, discovery_factory)
                # Vacuity guard: this mode exists to put a *bare
                # subscriber* in front of a pool. If `_resolve_borrower`
                ...
                # borrower: the context-manager forms are single-use, and
                # a `@contextmanager` object is itself callable, so
                # calling the live one would either consume it or fail.
                probe = _resolve_borrower(namespace, DiscoveryFactory.LOCAL_DIRECT)
