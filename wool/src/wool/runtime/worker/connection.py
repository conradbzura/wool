...
class RpcError(Exception):
    ...
    **Worker-health exception contract.** Load-balancer strategies
    ...
    - Any other class — caller-fault (`UnexpectedResponse` for a
      malformed or unrecognized worker response, parse-phase
      failures re-raised as the original exception type, caller-side
      encode failures, programming bugs); the strategy
      **propagates** to the caller without touching the pool. A
      strict-mode chain-encode failure surfaces in this bucket as a
      `~wool.runtime.context.exceptions.ChainSerializationError` (or
      a `BaseExceptionGroup` wrapping one) and propagates unwrapped.
    ...
    **Constructor invariant.** ``code`` and ``details`` are both
    optional, but a real gRPC fault always carries ``details`` (the
    human-readable failure), with ``code`` set whenever the status is
    known; the all-``None`` form is reserved as a test sentinel.
    """
...
class WorkerConnection:
    ...
    **Usage:**
    ...
    async def _handshake(
        ...
        """Send the dispatch request and await the worker's acknowledgement.

        Caller is responsible for channel-permit and call-cancel
        lifecycle; `_execute` pins both on its exit stack so any
        failure here triggers the registered cleanup callbacks during
        unwind.
...
class _DispatchStream(Generic[_T]):
    ...
    async def _send_and_read(
        ...
        """Run the shared guard/write/read/finally choreography for one round-trip.
    ...
    async def _read_next(self) -> _T:
        ...
        Serves paths that have already written their own request.
        ...
        mutations and reset signals both ride back-propagation.
