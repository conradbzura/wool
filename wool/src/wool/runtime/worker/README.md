...
The handler's `wool.Context` is bound to the worker-loop task — the same instance is registered against the worker task, not copied — so mutations under the worker task are observable to the handler when it later snapshots the Context for back-propagation. Coroutines use `concurrent.futures.Future` to bridge the result back; async generators stream results via an `asyncio.Queue`.
