...
class _WorkerOutcome:
    ...
    context: protocol.Context = field(default_factory=lambda: protocol.Context())
...
def _response_from_outcome(
    ...
    Shared by the coroutine and streaming dispatch paths. Picks the
    ``exception`` or ``result`` oneof based on which field is set,
    serializes the payload, and attaches the worker's context
...
class WorkerService(protocol.WorkerServicer):
    ...
    async def dispatch(
        ...
        if request.task.HasField("serializer"):
            sniff = unpickle_serializer(request.task.serializer)
            ...
            with self._tracker(
                ...
                try:
                    ...
                    yield protocol.Response(
                        ...
                        context=work_ctx.to_protobuf(serializer=serializer),
                    )
        finally:
            if is_passthrough:
