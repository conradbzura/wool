...
class ContextAlreadyBound(RuntimeError):
    ...
    at creation time. A second binding attempt (or a race against
    the wool task factory's copy-on-fork seed) would silently stomp
...
def _register(task: asyncio.Task[Any], ctx: Context) -> None:
    """Pin *ctx* to *task* when the binding identity comes from
    outside the spawning scope (typically the wire).

    Use when a worker-loop task must run under a :class:`Context`
    whose ID was decoded from a dispatch frame rather than forked
    from the parent scope — the wool task factory's copy-on-fork
    seed cannot reach across that boundary, so the binding has to
    be installed explicitly.

    .. caution::
        :func:`_register` does not acquire :meth:`Context._guard`,
        so binding two concurrently-running tasks to the same
        :class:`Context` leaves both free to mutate its data dict
        and used-token set without protection. The caller assumes
        responsibility for the single-task-per-:class:`Context`
        invariant — use only when the wire-supplied
        :class:`Context` ID makes the worker-loop task the
        exclusive mutator for the routine's lifetime.

    Callers from :func:`wool_factory` are structurally safe: the
    *task* is a freshly-spawned :class:`asyncio.Task` whose object
    identity has never appeared in :data:`context_registry`, so the
    one-shot check below cannot trip in that flow.
...
class Context:
    ...
    def __reduce__(self) -> NoReturn:
        ...
        Mirrors :class:`contextvars.Context`. All three would yield
        a snapshot disconnected from any live state, which is
        uniformly a footgun. Callers wanting in-process duplication
        must use :meth:`Context.copy` explicitly.
        """
        raise TypeError(
...
def install_task_factory(
    ...
    *after* Wool's, Wool's seeding via :data:`context_registry` is
    ...
    def wool_factory(
        ...
        **kwargs: Any,
        ...
        if isinstance(context, Context):
            ...
            # the whole routine. Register the task in wool's parallel
            # registry so current_context() resolves correctly from
            # inside the wrapped body.
            task: asyncio.Task[Any] = inner(  # pyright: ignore[reportAssignmentType]
                loop, _wool_scoped(context, coro), **kwargs
            )
            _register(task, context)
            return task
        # No wool.Context (or stdlib context): forward stdlib context if
        # present, then fork-from-parent so the new asyncio task picks
        # up a forked wool.Context regardless of how it was created.
        if context is not None:
            kwargs["context"] = context
        task = inner(loop, coro, **kwargs)  # pyright: ignore[reportAssignmentType]
...
def create_task(
    ...
    *,
    ...
    single-task guard is held continuously across awaits and
    registers the task in wool's per-task registry.
