...
def load(fw):
    by = {}
    for line in open(f"{RESULTS}/rust_stack_memo_sweep.jsonl"):
        r = json.loads(line)
        if r["framework"] == fw:
...
def main():
    ...
    for i, s in enumerate(SHAPES):
        ...
        ax.plot(wx, wR, "o-", color=WOOL_C, lw=2, ms=6, label="Wool full Rust + #273")
    ...
    fig.suptitle(
        "Wool full Rust + #273 (Rust caller + Rust worker) vs Ray 2.56.0 — "
        "canonical shape ladder (W=4, p50, warm)",
        fontsize=12.5,
        fontweight="bold",
    )
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    out = f"{RESULTS}/rust_stack_memo_vs_ray.png"
    fig.savefig(out, dpi=140)
    print(f"wrote {out}")
    print(
        "g=0 wool-full-rust-memo / Ray:", {s: round(r, 2) for s, r in zip(shown, ratios)}
    )
